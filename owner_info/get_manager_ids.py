...
def get_manager_ids(year):
    filename = "./weekly_scoreboards/" + str(year) + "/week_1_scoreboard.json"
    with open(filename) as json_file:
        data = json.load(json_file)

    with open("./owner_info/email_mapping.json") as f:
    ...
    with open("./owner_info/email_mapping.json", 'w') as f:
...
def update_team_mapping(year):
    ...
    with open("./owner_info/team_mapping.json") as f:
        team_mapping = json.load(f)

    team_mapping[year] = ids
    print(f'Writing team ids for {year}')
    with open("./owner_info/team_mapping.json", 'w') as f:
