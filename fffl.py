...
def findGameID(year):
    ...
    with open("./league_info/league_id_mapping.json", "r") as m:
...
def updateScoreboards(year):
    ...
    if not os.path.exists('./weekly_scoreboards/'+str(year)):
        os.makedirs('./weekly_scoreboards/'+str(year))
    ...
    while week < 17:  # assumes 16 week-schedule
        ...
        with open('./weekly_scoreboards/'+str(year)+'/'+file_name, 'w+') as outfile:
            json.dump(r, outfile)
...
def parse_scores(year, week):
    ...
    team_numbers = {}
    with open("./owner_info/team_mapping.json", "r") as f:
        team_numbers = json.load(f)

    scores = []

    filename = ("./weekly_scoreboards/" + str(year) +
